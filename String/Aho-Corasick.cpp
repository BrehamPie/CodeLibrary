#include <bits/stdc++.h>
using namespace std;
/*emplace_back tries to create a new element in your vector, by calling the constructor for the new element with the arguments passed in. */
struct ahoCorasick
{
        ...
        while (q.size())
        {
            ...
            for (int c = 0; c < MXCHR; c++)
            {
                ...
                else
                {
                    ...
                    finLink[v] = endList[link[v]].size() ? link[v] : finLink[link[v]];
                    q.push(v);
                }
            }
        }
    ...
    int match_text(string &str) /// Iterate through the whole text and find all matchings
    {
        int cur = 0;
        for (char c : str)
        {
